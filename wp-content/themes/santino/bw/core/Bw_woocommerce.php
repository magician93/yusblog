...
class Bw_woo {
	...
	static function init() {
		...
		if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            ...
            add_action( 'init', array('Bw_woo', 'custom_fix_thumbnail') );
			
            # out of stock message
            //add_filter('woocommerce_sale_flash', array('Bw_woo', 'woocommerce_sale_flashmessage'), 10, 2);
		}
	}
    
    static function woocommerce_sale_flashmessage( $flash ) {
        global $product;
    ...
    static function custom_fix_thumbnail() {
        
        add_filter('woocommerce_placeholder_img_src', 'custom_woocommerce_placeholder_img_src');
       
        function custom_woocommerce_placeholder_img_src( $src ) {
            return Bw::empty_img('400x520');
        }
    }
	...
	static function woocommerce_header_add_to_cart_fragment( $fragments ) {
		...
		?>
        <a class="cart-contents dJAX_internal" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'woothemes'); ?>">
